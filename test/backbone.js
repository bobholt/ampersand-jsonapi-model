...
function module(moduleName, opts) {
  ...
  test.only = function(testName, cb) {
    ...
    tape.only(moduleName + ' - ' + testName, cb);
  };
...
(function() {
  ...
  const Proxy = Backbone.Model.extend();
  ...
  test('initialize with defaults', function(t) {
    ...
    const model = new Model({first_name: 'John'});
  ...
  test('set and unset', function(t) {
    ...
    a.validate = function(attrs) {
      t.equal(attrs.foo, undefined, 'validate:true passed while unsetting');
    };
    a.unset('foo', {validate: true});
    t.equal(a.get('foo'), undefined, 'Foo should have changed');
    delete a.validate;
  ...
  test('set falsy values in the correct order', function(t) {
    ...
    model.on('change', function() {
      t.equal(model._changed.result, undefined);
  ...
  test('validate after save', function(t) {
    ...
    model.sync = function(method, m, options) {
      // MODIFIED FOR JSON-API
      options.success.call(this, {data: {attributes: {admin: true}}});
      // END MODIFICATION
    };
  ...
  test('save, fetch, destroy triggers error event, calls error handler, both with proper args, when an error occurs', function(t) {
    t.plan(6);
    const model = new Backbone.Model();
    model.on('error', function(m, resp) {
      t.equal(resp, 'xhrError');
    });
    model.sync = function(method, m, options) {
      options.error('xhrError');
    };
    model.save({data: 2, id: 1}, {error(m, resp) {t.equal(resp, 'xhrError'); }});
    model.fetch({error(m, resp) {t.equal(resp, 'xhrError');}});
    model.destroy({error(m, resp) {t.equal(resp, 'xhrError');}});
  });
  ...
  test.skip('save with non-object success response', function(t) {
    ...
    model.save({testing: 'empty'}, {
      success(m) {
        t.deepEqual(m.attributes, {testing: 'empty'});
      },
    });
  });
  ...
  test.skip('save turns on parse flag', function(t) {
    t.plan(1);
    const Model = Backbone.Model.extend({
      sync(method, model, options) {t.ok(options.parse);},
    });
    new Model().save();
  });
  ...
  test('#1791 - `attributes` is available for `parse`', function(t) {
    const Model = Backbone.Model.extend({
      parse() {return this.attributes; }, // shouldn't throw an error
    });
